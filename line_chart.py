...
STOCK_PRICE_DATA = read_from_sql("SELECT date,close,stock FROM airflow_db.stock_history where stock= 'AMZN' and date >= '2024-01-01'")
STOCK_VOLUME_DATA = read_from_sql("SELECT date,volume, stock FROM airflow_db.stock_history where stock= 'AMZN' and date >= '2024-01-01'")    
...
def generate_stock_line_chart_and_underlying_data(stocks,start_date,end_date, first_load):
    if first_load:
...
def generate_stock_volume_line_chart_and_underlying_data(stocks,start_date,end_date, first_load):
    if first_load:
